...
const formatTime=(duration)=>{//for converting duration of song from millisecond to min and seconds
...
//Below function is used to show selection once we click on certain item.
...
const togglePlay=()=>{
    if(audio.src){
        if(audio.paused){// for playing stoping fuctionality
            audio.play();
...
//Below function is used to play songs
const playTrack = (event,{image,artistNames, name, duration,previewURL,id})=>{
    ...
    if (audio.src=== previewURL) {
        ...
        audio.src= previewURL;//Url for the song to be play
    
        audio.play();//This is used to play music
...
const loadPlaylistTracks=({tracks})=>{
    ...
    for(let trackItem of tracks.items.filter(item=> item.track.preview_url)){
        ...
        track.id=id;
        track.className="track p-1 grid grid-cols-[50px_2fr_1fr_50px] items-center justify-items-start rounded-md gap-4 hover:bg-light-black";
...
const fillContentForPlaylist=async(playlistId)=>{//for uploading particular playlist
    ...
    <section>
                ...
                <p id="playlist-artist">Artists</p>
...
document.addEventListener("DOMContentLoaded",async()=>{
    ...
    history.pushState(section,"","");//history api help us to move forward and back using -> and <- arrow .
    loadSection(section);
    ...
    audio.addEventListener("play",()=>{
        ...
        progressInterval=setInterval(()=>{
            ...
            songDurationCompleted.textContent=audio.currentTime.toFixed(0)<10?`0:0${audio.currentTime.toFixed(0)}`:`0:${audio.currentTime.toFixed(0)}`;//to update the progress bar
            songProgress.style.width = `${(audio.currentTime/audio.duration)*100}%`; //to fill the progress bar
        },100);
    ...
    window.addEventListener("popstate",(event)=>{//redirected to dashboard if press back button
